<template>
    <div class="saltwater-fish">
        ...
        <div class="saltwaterCard">
            <div class="saltwaterCard-box">
                <!-- 商品卡片 -->
                <div class="card-container" @click="toProductDetail(item)" v-for="item in saltwaterfish.list"
                    :key="item.id">
                    <div class="card">
                        <div class="front-content">
                            <div class="saltwaterCard-card">
                                <div class="saltwaterCard-card-img">
                                    <img :src="'http://localhost:3000' + item.image" :alt="item.name"></img>
                                </div>
                                <div class="saltwaterCard-card-content">
                                    <span class="saltwaterCard-card-content-title">
                                        {{ item.name }}
                                    </span>
                                    <span class="saltwaterCard-card-content-price">￥<span>{{ item.price }}</span></span>
                                </div>
                            </div>
                        </div>
                        <div class="content">
            ...
            <div class="pagination">
                <el-pagination background layout="prev, pager, next" :total="saltwaterfish.total" :page-size="10"
                    v-model:current-page="saltwaterfish.currentPage" @current-change="handlePageChange" class="mt-4" />
            </div>
        </div>
    </div>
</template>
...
const showSearchResultsPage = (page) => {
    ...
    // 更新当前页数据
    saltwaterfish.list = allSearchResults.value.slice(startIndex, endIndex);
    saltwaterfish.currentPage = page;
    // 更新总记录数
    saltwaterfish.total = allSearchResults.value.length;
}
const fetchData = async (page) => {
    try {
        if (isSearchMode.value && input.value) {
            ...
            // 普通模式：获取当前页数据
            const dataRes = await api.getSaltwaterfish({ page: page })
            if (dataRes.data.status === 200) {
                saltwaterfish.list = dataRes.data.data
                saltwaterfish.currentPage = page
            }

            // 获取总记录数
            const totalRes = await api.getSaltwaterfishtotal()
            if (totalRes.data.status === 200) {
                saltwaterfish.total = totalRes.data.data
            }
        }
    } catch (error) {
        ...
        .saltwaterCard-box {
            ...
            .saltwaterCard-card {
                ...
                .saltwaterCard-card-content {
                    ...
                    .saltwaterCard-card-content-title {
                        font-size: 20px;
                        font-weight: bold;
                    }

                    .saltwaterCard-card-content-price {
                        font-weight: bold;
                        font-size: 15px;
