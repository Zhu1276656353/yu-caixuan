...
const http = (page = 1) => {
    api.getBackendGoodsFishtool({ params: { page } }).then(res => {
...
const handleDelete = (index, row) => {
    ElMessageBox.confirm(`确定删除商品 ${row.name} 吗？`, '删除商品', {
        ...
        api.getBackendGoodsFishtoolDelete({ id: row.id }).then(res => {
            if (res.data.status === 200) {
...
const sureUpdateGoodsHandler = () => {
    ...
    api.getBackendGoodsFishtoolUpdate(goodsUpdateForm.value).then(res => {
        if (res.data.status === 200) {
...
const searchHandler = () => {
    ...
    api.getBackendGoodsFishtoolSearch({ search: search.value }).then(res => {
        if (res.data.status === 200) {
            tableData.value = res.data.data;
