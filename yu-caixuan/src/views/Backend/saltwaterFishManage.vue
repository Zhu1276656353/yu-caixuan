...
const http = (page = 1) => {
    api.getBackendGoodsSaltwaterfish({ params: { page } }).then(res => {
...
const handleDelete = (index, row) => {
    ElMessageBox.confirm(`确定删除商品 ${row.name} 吗？`, '删除商品', {
        ...
        api.getBackendGoodsSaltwaterfishDelete({ id: row.id }).then(res => {
            if (res.data.status === 200) {
...
const sureUpdateGoodsHandler = () => {
    ...
    api.getBackendGoodsSaltwaterfishUpdate(goodsUpdateForm.value).then(res => {
        if (res.data.status === 200) {
...
const searchHandler = () => {
    ...
    api.getBackendGoodsSaltwaterfishSearch({ search: search.value }).then(res => {
        if (res.data.status === 200) {
            tableData.value = res.data.data;
